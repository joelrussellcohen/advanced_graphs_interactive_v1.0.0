...
make_unique_labels <- function(labels) {
  counts <- table(labels)
  duplicated_labels <- names(counts)[counts > 1]
  for (label in duplicated_labels) {
    indices <- which(labels == label)
    labels[indices] <- paste0(label, " (", seq_along(indices), ")")
  }
  return(labels)
}

# Fix duplicate label names
data_dictionary[data_dictionary$field_name %in% names(report_data), "field_label"] <- make_unique_labels(html_labels)
